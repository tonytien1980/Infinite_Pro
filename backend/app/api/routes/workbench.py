...
@router.get(
    ...
    response_model=schemas.PhaseSixCloseoutReviewResponse,
)
def get_phase_six_closeout_review_route(
    current_member=Depends(require_permission("access_firm_workspace")),
    db: Session = Depends(get_db),
) -> schemas.PhaseSixCloseoutReviewResponse:
    return get_phase_six_closeout_review(db)
...
@router.get(
    ...
    response_model=schemas.PhaseSixMaturityReviewResponse,
)
def get_phase_six_maturity_review_route(
    current_member=Depends(require_permission("access_firm_workspace")),
    db: Session = Depends(get_db),
) -> schemas.PhaseSixMaturityReviewResponse:
    return get_phase_six_maturity_review(db)
...
@router.get(
    ...
    response_model=schemas.PhaseSixReuseBoundaryGovernanceResponse,
)
def get_phase_six_reuse_boundary_governance_route(
    current_member=Depends(require_permission("access_firm_workspace")),
    db: Session = Depends(get_db),
) -> schemas.PhaseSixReuseBoundaryGovernanceResponse:
    return get_phase_six_reuse_boundary_governance(db)


@router.get(
    "/phase-6-generalist-guidance-posture",
    response_model=schemas.PhaseSixGeneralistGuidancePostureResponse,
)
def get_phase_six_generalist_guidance_posture_route(
    current_member=Depends(require_permission("access_firm_workspace")),
    db: Session = Depends(get_db),
...
@router.get(
    ...
    response_model=schemas.PhaseSixConfidenceCalibrationResponse,
)
def get_phase_six_confidence_calibration_route(
    current_member=Depends(require_permission("access_firm_workspace")),
    db: Session = Depends(get_db),
