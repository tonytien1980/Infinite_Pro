...
### 7.19 Phase 6 confidence calibration propagation

在 `confidence calibration v1` 已成立後，runtime 也已正式把這條 signal 回寫到既有 work-surface contract：
...
- 這一刀只在既有 second-layer guidance 補低噪音 `confidence calibration` note
- 不新增新頁面
- 不讓每個 page 自己額外查 `phase-6-confidence-calibration`
- 不把 second-layer note 膨脹成 calibration dashboard

因此這一層現在應被理解為：

- work-surface confidence-calibration propagation
