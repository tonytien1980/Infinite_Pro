# Phase 6 Persisted Governance Scoring / Sign-Off Foundation Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 讓 Phase 6 的 completion checkpoint 變成真正可保存的 score snapshot，並在 review-ready 後支援 owner-only sign-off。

**Architecture:** 延用既有 `WorkbenchExtensionState` phase-review payload。checkpoint 時保存 score snapshot；completion review 之後優先讀 persisted snapshot；新增 owner-only `phase-6-sign-off` route 與首頁 sign-off action，但不新增 handoff shell。
...
- phase-6 sign-off foundation
- homepage low-noise sign-off action / readout
...
- Modify: `docs/superpowers/specs/2026-04-07-phase-6-persisted-governance-scoring-sign-off-foundation-design.md`

### Task 1: Backend contract and sign-off tests

- [x] Add failing backend tests for `POST /workbench/phase-6-sign-off`.
- [x] Add failing backend tests for persisted checkpoint snapshot readback.
- [x] Implement schema / service / route changes.
- [x] Re-run targeted backend tests.

### Task 2: Frontend low-noise sign-off readout

- [x] Add failing frontend helper tests for phase-6 sign-off labels.
- [x] Implement type / api / helper / homepage sign-off action and signed-off readout.
