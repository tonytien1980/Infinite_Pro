...
- minimal adoption-feedback foundation 已開始成立：
  ...
  - 這是未來 precedent / reusable intelligence 的前置地基，不等於 precedent 系統已完成
...
- `Research / Investigation` 已從 first pass 補到較成熟的 guidance contract：
  - 不只會提示研究深度與第一個子題
  - 也會正式描述來源品質、新鮮度、矛盾訊號、citation-ready handoff 與缺口收斂計畫
  - `matter / task / evidence / deliverable` 也開始用共用 second-layer research helper 對齊研究閱讀順序
  - 但仍未做成獨立 research console，表面仍維持低噪音
- `follow_up / continuous` 已有正式 baseline；其中 `follow_up` 的 middle-layer clarity 已開始深化，但整體 retained advisory experience 仍未完成
- `continuous advisory` 已開始進入 MVP：
  - `matter workspace` 已是 retained-advisory 主控面
  - `task / deliverable / evidence` 已補上低噪音 continuity 對齊摘要
  - 第二層 continuity 閱讀也開始共用同一條 checkpoint / progression 語言
  - 但仍未完成完整 recurring review / outcome operating loop
- retained advisory MVP 現已能在 `continuous` matter workspace 低噪音回答：
  - 現在健康嗎
  - 最近推進到哪裡
  - 下一步先做什麼
  - 這輪結果目前追到哪裡
  - 下一次應該什麼時候回看
- `follow_up` 也開始補上 checkpoint timeline 與回來更新節奏：
  - 最近幾輪 checkpoint 怎麼變
  - 目前 checkpoint 節奏是否已站穩
  - 下次何時值得回來更新
- precedent / reusable intelligence 尚未正式成立；目前只有最小 adoption-feedback foundation
