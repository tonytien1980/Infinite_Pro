...
- `continuous` 現已開始進入 retained advisory MVP：
  ...
  - 第一波可見 UX 先集中在 matter workspace，而不是把所有工作面一起做成 dashboard
...
- `continuous advisory` 已開始進入 MVP，但目前仍只先完成 matter-first retained advisory surface，尚未完成完整 recurring review / outcome operating loop
