...
Infinite Pro 的 `/new` 現在正式承接三種進件模式，且三者都會匯進同一條正式主鏈，而不是三套互不相容的流程。

### 2.1 快速提問模式
- 使用者只輸入一句核心問題即可開案
- 不要求先上傳檔案
- 系統先建立 `Task / DecisionContext` 的初始骨架
- 後續可在案件工作台或來源 / 證據工作面補件

### 2.2 單文件模式
- 建立當下以一份主文件作為第一份正式 `SourceMaterial`
- 主文件與同一案件 / task 正式關聯
- 後續仍可補件，但建立時不混用多個網址或額外補充文字
...
- 可混合：
  ...
  - URL
  - 補充文字
- 後續仍可持續補件，不限制為建立當下的一次性附件集合

---

## 3. 正式工作主鏈

這三種進件模式都必須匯進同一條正式工作主鏈：
