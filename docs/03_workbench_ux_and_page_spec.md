...
- overview、案件主控台、建立新案件、設定頁，以及主要 detail workspace 必須能在手機尺寸下完成核心閱讀與主操作
...
- 對高流量第一層 surface，正式用字應優先採：
  ...
  - `現在重點`
  - `目前卡住原因`
- 上述簡化名稱屬於「可見第一層名稱」，只用於第一層導讀、hero 與主要 CTA
- formal/internal contract names 仍可保留較完整定義，例如 `matter workspace`、`artifact / evidence workspace`、`task workspace`、`deliverable workspace`
- visible first-layer labels 不等於 canonical contract names；第一層先服務顧問理解，formal/internal contract names 則保留給第二層、文件與系統內部對齊

正式實作補充：

- 共享 `frontend/src/lib/product-language.ts` 先集中定義第一層禁用詞與替代表，讓規則先有單一來源
- 後續高流量 surface / shared label helper 再逐步接入這層規則，避免各頁面各自長出不同翻譯
...
### 2.3.1 intake/list/return-path 的第一層命名規則

Task 3 起，`/new`、高流量列表頁與回跳入口的第一層文案，正式補上以下規則：

- intake 首屏不要再說 `正式進件主線`
  - 第一層改用：`不用先選模式，先說明問題就能開始`
- `matters`、`history` 這類高流量清單，不要再把第一層主要入口叫做 `工作紀錄`
  - 第一層改用：`分析項目`
- `deliverables` 第一層 list / nav / hero，不要只用內部感較重的 `交付物`
  - 第一層優先用：`結果與報告`
- `matters` 第一層入口與回跳，不要再把使用者送回 `案件工作面`
  - 依場景改用：`案件頁` 或 `案件主控台`
- intake 建立後若要補材料，不要再說 `來源與證據工作面`
  - 第一層改用：`資料與證據`
- 若從 history / list / deliverable 要引導使用者回主線，不要再說 `回案件工作面`、`回交付物工作面`
  - 第一層改用：`回案件頁`、`回案件主控台`、`回結果與報告`
- overview 首屏若要快速回看最近輸出或最近動作，不要再說 `最近交付物`、`工作紀錄`、`工作更新`
  - 第一層改用：`最近結果與報告`、`分析項目`、`分析更新`

正式規則：

- 這些命名屬於第一層導讀、hero、list label、主要 CTA 與 return path 規則
- 第二層文件與 internal contract names 仍可保留較完整定義，但不可反向污染第一層入口
...
- 若 calibration-aware weighting 已 propagation 到 task / matter / deliverable，第一波也只可補在既有 second-layer cards 內，不可新增 weighting 區塊牆
...
- 若 condensed second-layer note 同時有 freshness / lifecycle 與 boundary 類訊號，應優先讓顧問看到 freshness / lifecycle posture
...
- 若 closure criteria 已成立，也可在同一個 `Generalist Governance` 內補一塊 `completion review`：
...
- evidence 首屏只保留一個補件主問句與一個補完回跳點；首屏不再展示來源材料數、工作物件數、證據鏈數或連續性姿態的數字列，這些內容全部退到第二層
- evidence hero 只分兩塊：
  - 左側主卡：先判斷夠不夠，再決定要不要補
  - 右側回跳卡：補完後回哪裡，只保留一個清楚的 return cue
- 首屏不要再出現第三張同權重卡片、也不要在 button row 再放一次回跳入口
...
- 第一層 visible copy 不可再露出 `shared intelligence` 等混語；應直接使用 `共享判讀`
- Google 登入成功後，應直接返回 frontend 首頁，由既有 session gate 接手，不可停在 backend callback JSON
- 若使用者是從受保護頁面被導回 `/login?next=...`，Google 登入完成後應回到同一個安全的 frontend internal path；不可忽略 `next`、也不可接受外部網址
...
- `/demo` 的 backend seed 文案就應直接使用 visible product language；frontend normalization 只可當 guard，不可把 mixed-language seed 長期當正式來源
...
- 若跨案件背景目前仍偏薄，UI 也只可低噪音讀成：
  - `來源狀態：先留作背景參考`
- 若 backend 已開始正式提供 organization-memory posture，UI 也只可低噪音補：
...
- Evidence -> `證據`
- History -> `歷史紀錄`
- Matter / Engagement Workspace -> `案件主控台`
