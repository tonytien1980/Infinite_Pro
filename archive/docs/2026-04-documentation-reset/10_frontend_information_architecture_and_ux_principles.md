...
2. **建立新案件 / 新任務應是獨立入口或獨立工作頁**
   ...
   - UI 上只應保留一個 unified intake surface
   - 系統內部仍可推導三種 intake patterns：
     - 只有一句話
     - 一句話 + 1 份材料
     - 一句話 + 多份材料
   - 這三種 intake patterns 都必須匯進同一個案件主鏈，而不是三套互不相容的 wizard
...
2. **案件工作台**
   ...
   - `continuous` 首屏應可快速看出最新 progression state、上一個 progression snapshot、目前 action / outcome 狀態，以及下一步最建議做什麼
...
3. **交付物**
   ...
   - 若是 `continuous`，還應可看出它承接哪一個 progression state、最近 action / outcome 有何變化，以及下一步應回案件工作台更新 progression、先補件，還是刷新 deliverable
   - 它不是單次 task 的結果區替身
...
4. **代理管理**
   ...
   - 建立代理不等於案件一定會使用該代理；是否真正拉進任務執行，仍由 Host 在案件脈絡中判斷
...
5. **模組包管理**
   ...
   - 建立模組包不等於案件一定會套用該 pack；是否真的進入任務執行，仍由 Host 在案件脈絡中判斷
   - 它應分為兩個頁籤：
     - 問題面向模組包
     - 產業模組包
...
正式規則：
- 三種 intake patterns 共用同一條 task / matter / source material / evidence / deliverable 主鏈
- intake 第一站應先進入 `Case World Compiler`
- 系統應根據主問題與材料組成自動判讀 sparse inquiry、單材料起手或 multi-source case
...
9. **light mode 與 dark mode 都屬正式工作台主題**
   - 兩者都應是正式可用主題，而不是其中一個只是裝飾或半成品
   - light mode 應先建立穩定閱讀層級，再讓 dark mode 保持相同的結構、權重與可讀性
   - 不可把 dark mode 做成炫技型 compute 介面而犧牲正式產品感與可讀性
