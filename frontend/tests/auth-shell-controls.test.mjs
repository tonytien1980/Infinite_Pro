...
test("new matter intake uses an explicit upload trigger instead of a bare visible file input", () => {
  ...
  assert.match(source, /sourceFilesInputRef/);
  assert.match(source, /sourceFilesInputRef\.current\?\.click\(\)/);
...
test("evidence supplement flow also uses an explicit upload trigger", () => {
  ...
  assert.match(source, /選擇檔案/);
  assert.match(source, /matterFilesInputRef/);
  assert.match(source, /matterFilesInputRef\.current\?\.click\(\)/);
